...
- [Client/Server Runtime Subsystem `csrss.exe`]([Client/Server Runtime Subsystem](https://en.wikipedia.org/wiki/Client/Server_Runtime_Subsystem)
...
Las variables de entorno explican la configuración del host. `set` imprime todas. Una variable clave es `PATH`: Windows busca ejecutables primero en el directorio de trabajo actual (CWD) y luego en las rutas de `PATH` en orden. Si una carpeta colocada en `PATH` es escribible por el usuario y está antes de `C:\Windows\System32`, puede permitir “DLL planting” o ejecución de un binario malicioso sin especificar ruta completa.
